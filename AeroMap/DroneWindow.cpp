...
const int THUMB_WIDTH = 136;		// width of thumbnails, pixels
...
int DroneWindow::LoadImageList()
{
	...
	for (int i = 0; i < list.size(); ++i)
	{
		...
		if (fileInfo.isFile())
		{
			...
			double scale = (double)w / (double)THUMB_WIDTH;
			int destx = 0;
			for (double x = 0; x < w; x += scale)
			{
				int desty = 0;
				for (double y = 0; y < h; y += scale)
				{
					QColor c = image.pixelColor((int)x, (int)y);
					vx.r = c.redF();
					vx.g = c.greenF();
					vx.b = c.blueF();
					vx.x = destx;
