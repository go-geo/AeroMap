...
TerrainWindow::TerrainWindow(QWidget* parent, const char* terrainFile)
	...
	, m_FrameBuffer(-1)
{
	...
	ms_TerrainFile = terrainFile;

	SetCurrentFile(terrainFile);
}
...
void TerrainWindow::paintGL()
{
	...
	{
		if (ms_TerrainFile.GetLength() > 0)
		{
			mp_Terrain = new TerrainGL(ms_TerrainFile.c_str());
			OnViewTop();
			GLManager::CheckForOpenGLError(__FILE__, __LINE__);
		}
	}
...
void TerrainWindow::RenderText(QPainter* pPainter)
{
	...
	sprintf(buf, "File: %s", ms_TerrainFile.c_str());
...
//	// excavate the patch
//
//	GetApp()->LogWrite("Excavating region ...");
//
...
//	// update the texture
//
//	GetApp()->LogWrite("Updating texture ...");
//
...
//	update();
//
//	GetApp()->LogWrite("Excavation complete");
//}
...
bool TerrainWindow::Save()
{
	if (mp_Terrain)
	{
		GetApp()->LogWrite("Saving terrain...");
		mp_Terrain->Save();
		SetCurrentFile(mp_Terrain->GetFileName().c_str());
		GetApp()->LogWrite("Terrain saved");
	}
	return true;
}
...
{
	return StrippedName(ms_TerrainFile.c_str());
}
...
{
	// Return true if ok to close window.
...
void TerrainWindow::Contour()
{
	...
	if (mp_Terrain)
	{
		...
		{
			GetApp()->LogWrite("Creating contour lines ...");
			...
			GetApp()->LogWrite("Contour complete");
		}
	}
	update();
}
...
void TerrainWindow::SmoothElevation()
{
	...
	GetApp()->LogWrite("Applying smoothing ...");
	...
	GetApp()->LogWrite("Smoothing complete");
}
...
void TerrainWindow::ChangeElevation(double delta, double scaleFactor, bool isDelta)
{
	...
	GetApp()->LogWrite("Applying elevation ...");
	...
	GetApp()->LogWrite("Elevation complete");
}
...
void TerrainWindow::CreateActions()
{
	...
	mp_actionProperties->setStatusTip(tr("Show terrain properties"));
...
void TerrainWindow::OnProperties()
{
	if (mp_Terrain)
	{
		XString meta = mp_Terrain->GetMetaData();
...
TerrainGL* TerrainWindow::GetTerrain()
{
	return mp_Terrain;
}
