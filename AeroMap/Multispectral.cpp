...
VEC3 Multispectral::vignette_map(Photo* photo)
{
...
XString Multispectral::get_primary_band_name(std::vector<Reconstruction::MultiType> multi_camera, XString user_band_name)
{
	XString band_name;
	...
	{
		band_name = multi_camera[0].band_name;
	}
	else
	{
	...
	{
		band_name = multi_camera[0].band_name;
		Logger::Write(__FUNCTION__, "Cannot find band name '%s', will use '%s' instead", user_band_name.c_str(), band_name.c_str());
	}
...
//void Multispectral::compute_homography(XString image_filename, XString align_image_filename)
//{
//    try:
