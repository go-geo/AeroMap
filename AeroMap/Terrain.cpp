...
Terrain::Terrain(const char* terrainFile)
	: ms_TerrainFile(terrainFile)
	, m_File(0)
	, mb_IsDirty(false)
{
	// must have valid terrain to instantiate terrain class
	assert(terrainFile);
	assert(terrainFile[0] != 0);
...
XString Terrain::GetMetaData()
{
	...
	str = XString::Format("NE Coordinates: %0.1f, %0.1f\n", m_Header.NEX, m_Header.NEY); strMeta += str;
