...
int Project::LoadImageList()
{
	...
	for (int i = 0; i < list.size(); ++i)
	{
		...
		if (fileInfo.isFile())
		{
			if (Photo::is_supported_format(file_name) == false)
				continue;
...
{
	return XString::CombinePath(tree.georef_path, "coords.txt");
}
...
int Project::GetUndistMaxImageSize()
{
	return m_undist_image_max_size;
}

void Project::SetUndistMaxImageSize(int max_size)
{
	m_undist_image_max_size = max_size;
}
...
void Project::InitArg()
{
	...
	// --sfm-no-partial      
	//					Do not attempt to merge partial reconstructions. This can happen when images do not have
	...
	// --rolling-shutter
	//					Turn on rolling shutter correction. If the camera has a rolling shutter and the images were
...
void Project::InitTree()
{
	...
	tree.filter_path = XString::CombinePath(ms_DroneOutputPath, "odm_filterpoints");
	...
	tree.mesh_path = XString::CombinePath(ms_DroneOutputPath, "odm_meshing");
	tree.mesh = XString::CombinePath(tree.mesh_path, "odm_mesh.ply");
	tree.mesh_25d = XString::CombinePath(tree.mesh_path, "odm_25dmesh.ply");

	tree.texture_path = XString::CombinePath(ms_DroneOutputPath, "odm_texturing");
	tree.texture_25d_path = XString::CombinePath(ms_DroneOutputPath, "odm_texturing_25d");
	tree.textured_model_obj = "odm_textured_model_geo.obj";
	tree.textured_model_glb = "odm_textured_model_geo.glb";
	...
	tree.georef_gcp = "";
	tree.georef_gcp_utm = XString::CombinePath(tree.georef_path, "gcp_list_utm.txt");

	tree.dem_path = XString::CombinePath(ms_DroneOutputPath, "odm_dem");

	tree.ortho_path = XString::CombinePath(ms_DroneOutputPath, "odm_orthophoto");
	tree.orthophoto_tif = XString::CombinePath(tree.ortho_path, "odm_orthophoto.tif");
	tree.orthophoto_render = XString::CombinePath(tree.ortho_path, "odm_orthophoto_render.tif");
	tree.orthophoto_log = XString::CombinePath(tree.ortho_path, "odm_orthophoto_log.txt");
	tree.orthophoto_corners = XString::CombinePath(tree.ortho_path, "odm_orthophoto_corners.txt");

	tree.report_path = XString::CombinePath(ms_DroneOutputPath, "odm_report");

	// executables

	XString odm_lib = GetApp()->GetProgLibPath();
	...
	tree.prog_gdal_info = XString::CombinePath(odm_lib, "venv/Lib/site-packages/osgeo/gdalinfo.exe");
	tree.prog_gdal_translate = XString::CombinePath(odm_lib, "venv/Lib/site-packages/osgeo/gdal_translate.exe");
	tree.prog_gdal_warp = XString::CombinePath(odm_lib, "venv/Lib/site-packages/osgeo/gdalwarp.exe");
}
