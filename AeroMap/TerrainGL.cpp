...
void TerrainGL::Render(glm::mat4& matModel, glm::mat4& matView, glm::mat4& matProj, Camera& camera, 
					   int defaultFBO, SizeType winSize)
{
...
void TerrainGL::SetPixel(UInt16 x, UInt16 y, PixelType pix)
{
	...
	// current design does not write to this
	assert(false);
...
void TerrainGL::UpdateTexture()
{
	// Create texture

	int size = GetColCount() * GetRowCount() * 4;
	UInt8* pData = new UInt8[size];

	if (m_texTerrain.Create(GetColCount(), GetRowCount(), pData) == false)
	{
		Logger::Write(__FUNCTION__, "Unable to create terrain texture.");
	}

	if (m_texTerrain.IsValid() == false)
		return;

	double min_elev = GetMinElev();
	double max_elev = GetMaxElev();
	for (int x = 0; x < GetColCount(); ++x)
	{
		for (int y = 0; y < GetRowCount(); ++y)
		{
			...
			double elev = GetHeight(y, x);
			double sf = (elev - min_elev) / (max_elev - min_elev);
			...
			int dest_row = GetRowCount() - y - 1;
			m_texTerrain.SetPixel(x, dest_row, pix);
		}
	}
...
void TerrainGL::SetColorScale(XString scale_file)
{
	...
	// simply deleting the color scale will give us gray
	if (scale_file != TerrainGL::SCALE_GRAY)
		mp_ScaleColor = new ScaleColor(scale_file.c_str());

	UpdateTexture();
}

void TerrainGL::ClearColorScale()
{
	// Clear the color scale. Results
	// in default use of grayscale.
	//
...
void TerrainGL::SetTextureFile(XString texture_file)
{
	// Override color scale to use texture instead.
	//

	if (m_texTerrain.Load(texture_file.c_str()) == false)
	{
		Logger::Write(__FUNCTION__, "Unable to load file: '%s'", texture_file.c_str());
	}
}
